...
/*
 * before saving a post, get the ${post_type}_meta from the post body
 * and add it to the post by storing it under a post-type specific key
 */
function add_insert_attribute_hooks( $prepared_post, $request, $success ) {
    if( $prepared_post->post_type ) {
        ...
        if( $meta_data ) {
            // get the existing post meta so we can mutate the current values, or unset something if we get a null value
            ...
            // overwrite the post meta with the new params
...
function prepare_metadata_init_hooks() {
    // iterate over all publicly available post types and register the hooks on that type...
